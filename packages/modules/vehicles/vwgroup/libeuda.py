#!/usr/bin/env python3
"""Client for the VW Group EU Data Act portal (OIDC login + data delivery)."""
...
UTC = None
KEEP_JSON = 30
DATA_BASE_PATH = Path(__file__).resolve().parents[4] / "data" / "modules"
JSON_BASE_PATH = Path(str(RAMDISK_PATH) + '/vweuda')
storeFileName = '/data_'
...
def get_brand_for_vin(vin: str, default: str = DEFAULT_BRAND) -> str:
    """Return a supported EUDA brand for a VIN prefix, falling back safely."""
    if not vin:
        return default
    return VIN_BRAND_MAP.get(vin[0:3].upper(), default)


def get_data_path(module_type: str) -> Path:
    """Return the persistent EUDA data path for an openWB vehicle module."""
    return DATA_BASE_PATH / module_type
...
class euda():
    ...
    def save_json_file(self, _name: str, vin: str, _data: dict) -> bool:
        ...
        _l = glob.glob(str(JSON_BASE_PATH) + '/*_' + vin + '.json')
        _l.sort()
        _len = len(_l)
        _del = _len - KEEP_JSON
    ...
    async def async_eudaThread(self, username: str, password: str, vin: str):
        ...
        try:
            async with aiohttp.ClientSession(headers={'Connection': 'keep-alive'}) as session:
                ...
                client_key = f"{username}:{brand}"
                if client_key not in euda.client:
                    _LOGGER.debug(f"create new client, key={client_key}")
    ...
    async def get_status(self,
        ...
        try:
            module_type = getattr(conf, 'type', 'vwgroup')
            data_path = get_data_path(module_type)
